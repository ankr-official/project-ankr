...
export default function Admin() {
  ...
  const handleToggleConfirm = async (event, e) => {
    e.stopPropagation();
    try {
      await update(ref(database, `data/${event.id}`), {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await remove(ref(database, `data/${deleteTarget.id}`));
  ...
  const handleSave = async (eventData) => {
    ...
    try {
      if (id) {
        await update(ref(database, `data/${id}`), data);
        ...
        await set(ref(database, `data/${newKey}`), data);
  ...
  const handleApproveReport = async (report) => {
    try {
      ...
      const newKey = `row${maxRow + 1}`;
      await set(ref(database, `data/${newKey}`), { ...data, confirm: false });
    ...
    <div className="min-h-screen bg-indigo-50/50 dark:bg-[#242424] transition-colors">
      ...
      <header className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-200/70 dark:border-gray-800 transition-colors">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            >
              <img src="/favicon.svg" alt="ANKR" className="w-6 h-6" />
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex overflow-x-auto gap-1 p-1 rounded-xl bg-gray-200/60 dark:bg-gray-800/60 shrink-0 self-start">
            {tabs.map((t) => (
              <button
                key={t.key}
          ...
          <div
            className={`relative flex-1 ${tab === "reports" ? "hidden" : ""}`}
          >
            <svg
                  ...
                  >
                    ...
                    {(report.time_start ||
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {[
                          report.time_start &&
                            `시작 ${new Date(report.time_start).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}`,
                          report.time_entrance &&
                            `입장 ${new Date(report.time_entrance).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}`,
                          report.time_end &&
                            `종료 ${new Date(report.time_end).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}`,
                        ]
                          .filter(Boolean)
                          .join(" · ")}
                      </p>
